...
<div class="min-h-screen pb-8">
  ...
  >
    <!-- Decorative circles -->
    <div
      class="absolute top-0 right-0 w-64 h-64 bg-sage/10 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2"
    ></div>
    <div
      class="absolute bottom-0 left-0 w-48 h-48 bg-peach/20 rounded-full blur-3xl translate-y-1/2 -translate-x-1/2"
    ></div>
    ...
    <div class="relative max-w-lg mx-auto">
      ...
      <div class="text-center mb-6">
        <p class="text-sm text-sage/80 font-medium tracking-wide">
          {getGreeting()}, {data.user?.displayName ||
      ...
      <div class="flex flex-col items-center">
        ...
        <div class="relative mb-4">
          ...
          <div class="absolute inset-0 scale-150">
            <div
              class="absolute inset-0 bg-sage/10 rounded-full blur-3xl animate-breathe"
            ></div>
          </div>
          <div class="absolute inset-0 scale-125">
            <div
              class="absolute inset-0 bg-sage/20 rounded-full blur-2xl animate-breathe"
          ...
          <!-- Main tree container -->
          <div
            class="relative w-36 h-36 rounded-full bg-gradient-to-br from-white to-sage/5 border border-sage/10 shadow-2xl shadow-sage/20 flex items-center justify-center"
          >
            <span
              class="text-7xl transform hover:scale-110 transition-transform duration-300 cursor-default select-none animate-pulse-soft"
            >
              {stage.symbol}
            </span>
          </div>
          ...
          {#if isMaxStage}
            <div class="absolute -bottom-2 left-1/2 -translate-x-1/2">
              <div
                class="px-3 py-1 bg-gradient-to-r from-amber-400 to-orange-400 text-white text-xs font-bold rounded-full shadow-lg"
              >
                ✨ MAX LEVEL
              </div>
            </div>
          ...
          <div class="w-full max-w-xs space-y-2">
            ...
            <div class="flex justify-between items-center text-xs">
              ...
              <span class="text-sage font-medium">
                {stage.maxMinutes + 1 - data.totalMinutes} min → {$t(
                  `tree.${nextStage.id}.name`,
                )}
              </span>
            </div>
          </div>
  ...
  >
    {#if meditatedToday}
      <div
        class="bg-gradient-to-r from-sage/10 to-sage/5 border border-sage/20 rounded-2xl p-4 flex items-center gap-4"
      >
        <div
          class="w-12 h-12 rounded-xl bg-sage/20 flex items-center justify-center text-2xl"
        >
          ✅
        </div>
        <div class="flex-1">
    ...
    {:else}
      <div
        class="bg-gradient-to-r from-peach/20 to-peach/10 border border-peach/30 rounded-2xl p-4 flex items-center gap-4"
      >
        <div
          class="w-12 h-12 rounded-xl bg-peach/30 flex items-center justify-center text-2xl animate-bounce-short"
        >
          🧘
        </div>
        <div class="flex-1">
        ...
        <Button
          variant="default"
          size="sm"
          class="rounded-full shadow-lg shadow-sage/30"
        >
  ...
  >
    <div class="grid grid-cols-3 gap-3">
      <!-- Streak -->
      <div
        class="group relative bg-white rounded-2xl p-4 border border-earth/10 shadow-sm hover:shadow-md hover:border-sage/20 transition-all duration-300 hover:-translate-y-0.5"
      >
        <div
          class="absolute top-3 right-3 text-2xl opacity-20 group-hover:opacity-40 transition-opacity"
        >
          🔥
        </div>
        ...
        {#if data.streak >= 7}
          <div
            class="absolute -top-1 -right-1 w-5 h-5 bg-orange-400 rounded-full flex items-center justify-center text-[10px] border-2 border-white"
          >
            🎯
          </div>
        {/if}
      ...
      >
        ...
        <p class="text-3xl font-bold text-sage mb-1">
  ...
  <section class="max-w-lg mx-auto mb-10 px-4">
    <a
      href="/timer"
      class="w-full py-4 text-lg font-semibold rounded-2xl shadow-lg hover:shadow-xl transition-all hover:scale-[1.02] active:scale-[0.98] bg-sage text-white flex items-center justify-center gap-3"
    >
    ...
    <div class="flex items-center justify-between mb-4 px-1">
      ...
      {#if nextBadge}
        <div class="text-right">
          <p class="text-[10px] text-slate/40 uppercase tracking-wider">
            {$t("badges.next")}
          </p>
          <p class="text-sm font-medium text-sage">
            {$t(`badges.${nextBadge.id}.name`)}
          </p>
        </div>
      {/if}
    </div>
        ...
        {#each achievements as badge}
          ...
          >
            <div class="p-4 text-center">
              ...
              {#if badge.achieved}
                <span
                  class="inline-flex items-center gap-1 text-[10px] text-sage font-medium"
                >
  ...
  >
    <div
      class="bg-white rounded-3xl border border-earth/10 shadow-sm overflow-hidden"
    >
      <!-- Calendar Header -->
      <div
        class="flex justify-between items-center p-4 border-b border-earth/5"
      >
        ...
        <div class="flex items-center gap-2">
          ...
          <span
            class="font-semibold text-sm text-sage min-w-[120px] text-center"
          >
      ...
      <div class="p-4">
            ...
            {#if day}
              ...
              <div
                class="aspect-square rounded-lg flex flex-col items-center justify-center relative group cursor-default transition-all duration-200 hover:scale-110
                  ...
                  {minutes > 0
                  ? 'bg-sage text-white'
                  : 'bg-earth/5 text-slate/40 hover:bg-earth/10'}"
        ...
        <!-- Legend -->
        <div
          class="mt-4 flex items-center justify-center gap-2 text-[10px] text-slate/50"
        >
    ...
    <section class="max-w-lg mx-auto mb-8 px-1">
      ...
      <div class="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
        {#each Object.entries(data.typeDistribution) as [type, count]}
          <div
            class="flex-shrink-0 bg-white rounded-xl border border-earth/10 px-4 py-3 shadow-sm"
          >
      ...
      <div class="space-y-2">
        {#each data.recentSessions as session}
          ...
          >
            <!-- Session type icon -->
            <div
              class="w-10 h-10 rounded-xl bg-sage/10 flex items-center justify-center text-lg flex-shrink-0"
            >
            ...
            <div class="flex-1 min-w-0">
              <p class="text-sm font-semibold text-slate truncate">
                {session.sessionType}
              </p>
              <p class="text-[10px] text-slate/40">
                {session.completedAt
            ...
            <div class="flex items-center gap-2">
              ...
              {/if}
              <span
                class="font-mono font-bold text-sage text-sm min-w-[3rem] text-right"
              >
